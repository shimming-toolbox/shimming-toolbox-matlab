...
% DEBUG
% Params = ShimCalRri.declarecalibrationparameters20170418() ;
% [Shim.img, Shim.Hdr] = ShimCalRri.mapdbdi20170915( Params ) ;
...
% =========================================================================
function [img, Hdr] = mapdbdi20170915( Params ) 
%MAPDBDI
% 
% there are major discrepancies between predicted and observed
% field maps post-shimming with the RRI coil using the reference
% maps from 20170706. (E.g. shim050 -> 059p experiments) 
% The following is a sanity check on the reference maps.
%
% 1. Using the +/- shim current field maps, the background field
% (susceptibility field) is isolated (fieldBkgr variable below) and subtracted
% from the individual field maps to yield maps that consist solely of the 
% shim field (fieldShim variable below).
%
% fieldShim was then used as a field map input to our standard Rri shim optimization
% to double check that the optimized current was indeed the equal-and-opposite
% current to the current injected for the field map acquisition.
%
% (i.e. if the field map was acquired with -0.5 A, then the correct optimized
% current will be +0.5 A in the same channel.
%
% RESULT : PASS
%
% 2. The +/- fieldShim maps are treated as inspired/expired fields respectively
% and input into the 'augmented' optimization we are using for real-time shimming.
% Again, the output currents should be equal and opposite to the currents used
% for the field maps.
%
% RESULT : PASS
%   Even using some regularization, the output currents are basically as expected.
%   
Params.DataDir = [] ;
...
fieldBkgr = zeros( [90 28 80 24] ) ;
% field shift due strictly to the shim
fieldShim = zeros( [90 28 80 24] ) ;

Params.pathToShimReferenceMaps = '/Users/ryan/Projects/Shimming/Static/Calibration/Data/ShimReferenceMapsRri20170706.mat' ;
Shims     = ShimOptRri( Params ) ;

channelToBankKey = ShimComRri.getchanneltobankkey() ;

Params.regularizationParameter     = 0.1 ;

solutionVectors = zeros(24,2,24) ;
...
for iChannel = 1 : Params.nChannels 
    ...
    fieldBkgr(:,:,:, iChannel) = ( fieldMaps(:,:,:, 1 ) + fieldMaps(:,:,:,2) )/2 ;
    
    FieldIn = Field.copy() ;
    FieldEx = Field.copy() ;

    % Field from positive shim current  
    FieldIn.img = fieldMaps(:,:,:,2) - fieldBkgr(:,:,:,iChannel) ;
    
    % Field from negative shim current  
    FieldEx.img = fieldMaps(:,:,:,1) - fieldBkgr(:,:,:,iChannel) ;

    Shims.setoriginalfield( FieldIn ) ;

    Params.shimVoi = ( FieldIn.img ~= 0 ) ;
    Shims.setshimvolumeofinterest( Params.shimVoi ) ;

    currents = Shims.optimizeshimcurrents( Params ) ;
    display('Determine the row index of the max. abs. element of currents') 
    display('On the same row, the 4th column of channelToBankKey should indicate the row of the same max. element in CurrentsOut (mapped RRI style)')

    currents
    channelToBankKey
    [currentsOut] = ShimComRri.mapcurrentstomxd( currents ) ;
    currentsOut = [ [1:length(currentsOut)]' currentsOut ]

    display('Hypothetical inspired/expired:')
    [currentsIn, currentsEx] = Shims.optimizeshimcurrents( Params, FieldEx ) ;
    solutionVectors(:,:, iChannel) = [currentsIn currentsEx] 
end 
