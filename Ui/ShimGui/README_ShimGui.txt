...
Shim GUI is a graphic interface for shimming simulations and experiments. It allows users to display field and magnitude maps, select manually or automatically (Using Spinal Cord Toolbox) a VOI and simulate shimming in this region (Software). Then, users can open serial communication between the computer and the antenna to send shim currents (Hardware). Shim GUI was developed by NeuroPoly, the neuroimagery laboratory of Polytechnique Montréal.
